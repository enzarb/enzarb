...
async fn watch_path(
    ...
    loop {
        ...
        for event in events {
            ...
            let kind = if event.mask.contains(inotify::EventMask::CREATE) { "create" }
                else if event.mask.contains(inotify::EventMask::DELETE) { "delete" }
                else if event.mask.contains(inotify::EventMask::MODIFY) { "modify" }
                else if event.mask.contains(inotify::EventMask::MOVED_FROM) { "moved_from" }
                else if event.mask.contains(inotify::EventMask::MOVED_TO) { "moved_to" }
                else { "unknown" };
